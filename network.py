...
class Network:
    ...
    def get_networks(self, sub_filename, req_num, granularity=3):
    ...
    def get_reqs(self, req_num, granularity=3):
        ...
        for i in range(req_num):
            ...
            req = self.read_req_file(filename, granularity)
            req.graph['parent'] = -1
            req.graph['id'] = index
            queue.append(req)
        return queue

    def get_reqs_for_train(self, req_num, granularity=3):
        """读取req_num个虚拟网络请求文件，构建虚拟网络请求事件队列"""
        queue = []
        for i in range(req_num):
    ...
    def read_network_file(self, filename, granularity=3):
    ...
    def read_req_file(self, filename, granularity=3):
        """读取网络文件并生成networkx.Graph实例"""
        ...
        # Step 1: 获取网络节点数量和链路数量，并根据网络类型进行初始化
        node_num, link_num, time, duration, delay, jitter, packetloss, max_dis = [int(x) for x in lines[0].split()]
        graph = nx.Graph(type=0, time=time, duration=duration, delay=delay, jitter=jitter,
                         packetloss=packetloss, mapped_info=mapped_info)
